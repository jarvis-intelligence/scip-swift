...
**Re-baselining surface:** ONLY `IntegrationTests.swift:42-45` (`#expect(displayNames.contains("Greeter"))` / `("greet()")` / `("name")` — exact-match Set.contains breaks when displayName becomes `MiniSwiftPackage.Greeter`). `SCIPSymbolFormatterTests`, `CacheStoreTests`, `IndexManifestTests`, `ScipIndexMergerTests`: untouched. `IncrementalIntegrationTests`: no edit needed (equality assertions hold once demangling is deterministic), but must run green post-change.
