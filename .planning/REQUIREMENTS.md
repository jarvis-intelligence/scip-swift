...
- [ ] **SYMBOL-03**: Symbol identity remains stable — the wrapped USR stays the canonical `symbol` field so incremental cache hits and cross-repo merge dedup behave unchanged when demangling is enabled (verified by existing second-run byte-identity and merge tests, re-baselined).
- [ ] **SYMBOL-04**: User can disable demangling (`--no-demangle`) to reproduce v0.2.x-style opaque output.
